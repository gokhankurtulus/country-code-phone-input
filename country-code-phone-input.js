...
// input must be a valid number format or a modifier key, and not longer than ten digits
const enforceFormat = (event) => {
    const input = event.target.value.replace(/\D/g, '').substring(0, 10); // First ten digits of input only
    if (input.length === 10 && !isModifierKey(event)) {
        event.preventDefault();
    }
...
// for generate formatted input value
